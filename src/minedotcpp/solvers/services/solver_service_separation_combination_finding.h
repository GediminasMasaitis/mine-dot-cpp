	...
	namespace solvers
	{
		namespace services
		{
			...
			private:
				...
				void cl_validate_predictions(std::vector<unsigned char>& m, std::vector<int>& results, unsigned total) const;
#endif

				int find_hamming_weight(int i) const;
				void get_combination_search_map(solver_map& solver_map, border& border, std::vector<unsigned char>& m) const;
				void thr_validate_predictions(std::vector<unsigned char>& m, std::vector<int>& results, unsigned total) const;
				void validate_predictions(std::vector<unsigned char>& m, std::vector<int>& results, unsigned min, unsigned max, std::mutex* sync) const;
