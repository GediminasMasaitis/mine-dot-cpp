...
namespace minedotcpp
{
	namespace solvers
	{
		...
		private:
			...
			bool is_prediction_valid(solver_map& map, std::unordered_map<common::point, bool, common::point_hash>& prediction, std::vector<common::cell>& empty_cells) const;
			int SWAR(int i) const;
			void calculate_border_probabilities(border& b) const;
			void get_verdicts_from_probabilities(std::unordered_map<common::point, double, common::point_hash>& probabilities, std::unordered_map<common::point, bool, common::point_hash>& target_verdicts) const;
